<?php
    ...
    class universidad_model{
        ...
        function create($data){
            ...
            $sql="INSERT INTO universidad(nombre,ubicacion)VALUES (?,?)";

            $query = $this->DB->prepare($sql);
            $query->execute(array($data['nombre'],$data['ubicacion']));
            Database::disconnect();       
        ...
        function update($data,$date){
            $this->DB->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $sql = "UPDATE universidad  set  nombre=?, ubicacion =? WHERE id = ? ";
            $q = $this->DB->prepare($sql);
            $q->execute(array($data['nombre'],$data['ubicacion']));
            Database::disconnect();
